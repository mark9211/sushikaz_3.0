...
class Sales extends AppModel {
	...
	public function drinkCalculateAddTax($sales){
		$drink=0;$food1=0;$food2=0;
		foreach($sales as $sales_one){
			if($sales_one['Type']['id']==11||$sales_one['Type']['id']==13){
				$drink+=floor($sales_one['Sales']['fee']*1.08);
			}elseif($sales_one['Type']['id']==9||$sales_one['Type']['id']==10){
				$food1+=floor($sales_one['Sales']['fee']*1.08);
			}elseif($sales_one['Type']['id']==12){
				$food2+=floor($sales_one['Sales']['fee']*1.08);
			}elseif($sales_one['Type']['id']==14){
				$food1+=floor($sales_one['Sales']['fee']/2*1.08);
				$food2+=floor($sales_one['Sales']['fee']/2*1.08);
			}
		}
		return array('drink'=>$drink, 'food1'=>$food1, 'food2'=>$food2);
	}
